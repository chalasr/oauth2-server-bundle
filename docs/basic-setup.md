...
through the `client.hasher` option:
...
If you installed the bundle using [Symfony Flex](https://github.com/symfony/flex), an
equivalent override is provided out of the box for new installations. On existing projects,
run `composer recipes:update league/oauth2-server-bundle` to update your recipe, or add the
configuration above manually.

> [!TIP]
> Because client secrets generated by this bundle are high-entropy random values, they do
> not need a slow password-grade hash. Setting `client.hasher.algorithm` to `sha256` in
> production keeps secrets safely hashed while removing the per-request hashing cost
> entirely.
>>>>>>> Stashed changes
