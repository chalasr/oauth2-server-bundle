    ...
    private function createClientNode(): NodeDefinition
    {
        ...
        $node
            ...
            ->children()
                ...
                ->end()
                ->arrayNode('hasher')
                    ->info('Configures the hasher used to store client secrets.')
                    ->addDefaultsIfNotSet()
                    ->children()
                        ->scalarNode('algorithm')
                            ->info('The hashing algorithm. Use "auto" to let Symfony pick the best available one, or e.g. "bcrypt", "argon2id" or "sha256" for high-entropy secrets.')
                            ->defaultValue('auto')
                            ->cannotBeEmpty()
                        ->end()
                        ->integerNode('cost')
                            ->info('The bcrypt work factor (only used by the "bcrypt"/"native" algorithms).')
                            ->min(4)->max(31)
                            ->defaultNull()
                        ->end()
                        ->integerNode('memory_cost')
                            ->info('The Argon2 memory cost in KiB (only used by the "argon2*"/"sodium" algorithms).')
                            ->defaultNull()
                        ->end()
                        ->integerNode('time_cost')
                            ->info('The Argon2 time cost (only used by the "argon2*"/"sodium" algorithms).')
                            ->defaultNull()
                        ->end()
                    ->end()
                ->end()
